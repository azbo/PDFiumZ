using System;

namespace PDFiumCore.HighLevel;

/// <summary>
/// Thrown when rendering fails.
/// </summary>
public class PdfRenderException : PdfException
{
    /// <summary>
    /// Initializes a new instance of the <see cref="PdfRenderException"/> class.
    /// </summary>
    public PdfRenderException() { }

    /// <summary>
    /// Initializes a new instance of the <see cref="PdfRenderException"/> class with a specified error message.
    /// </summary>
    /// <param name="message">The message that describes the error.</param>
    public PdfRenderException(string message) : base(message) { }

    /// <summary>
    /// Initializes a new instance of the <see cref="PdfRenderException"/> class with a specified error message
    /// and a reference to the inner exception that is the cause of this exception.
    /// </summary>
    /// <param name="message">The error message that explains the reason for the exception.</param>
    /// <param name="inner">The exception that is the cause of the current exception.</param>
    public PdfRenderException(string message, Exception inner) : base(message, inner) { }
}
